...
/*
 * When warm colors are requested we keep the same hue mapping as the regular
 * color() function and simply reduce the green and blue components. This keeps
 * the nice cyclical gradient while shifting the palette towards reds without
 * redefining a dedicated gradient.
 */
vec3 warmify(vec3 c) {
    const float G_RATIO = 0.5;
    const float B_RATIO = 0.25;
